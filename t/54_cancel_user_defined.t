...
$slist->get_file_info( 'white.pnm', undef, undef, undef, sub {
 ...
 $slist->import_file( $info, 1, 1, undef, undef, undef, sub {
  $slist->user_defined( $slist->{data}[0][2], 'convert %i -negate %o', undef, undef, undef, undef, undef, undef, sub {
   is( -s 'white.pnm', -s "$slist->{data}[0][2]{filename}", 'image not modified' );
   Gtk2->main_quit;
  });
  $slist->{cancelled} = 1;
 })
