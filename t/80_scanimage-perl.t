...
Options specific to device `test':
  ...
  Bool test options:
    ...
    --bool-hard-select-soft-detect[=(yes|no)] [inactive]
        (2/6) Bool test option that has hard select and soft detect (and
        advanced) capabilities. That means the option can't be set by the
        frontend but by the user (e.g. by pressing a button at the device).
    --bool-hard-select[=(yes|no)] [inactive]
        (3/6) Bool test option that has hard select (and advanced)
        capabilities. That means the option can't be set by the frontend but by
        the user (e.g. by pressing a button at the device) and can't be read by
        the frontend.
    --bool-soft-detect[=(yes|no)] [inactive]
        (4/6) Bool test option that has soft detect (and advanced)
...
Type ``scanimage-perl --help -d DEVICE'' to get list of all options for DEVICE.
