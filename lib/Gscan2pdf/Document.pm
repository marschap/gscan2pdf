...
sub negate {
 ...
 my $started_flag;
 my $sentinel = _enqueue_request( 'negate', { page => $page->freeze } );
 $queued_callback->( $_self->{process_name}, $jobs_completed, $jobs_total )
   if ($queued_callback);
 _when_ready(
  $sentinel,
