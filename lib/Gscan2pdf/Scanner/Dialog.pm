...
BEGIN {
 use Gscan2pdf::Scanner::Options; # need to register this with Glib before we can use it below
}
...
sub INIT_INSTANCE {
 ...
 $spin_buttoni->signal_connect(
  'value-changed' => sub {
   $spin_buttoni->set_value( -$self->get('page-number-increment') ) if ( $spin_buttoni->get_value == 0 );
...
sub SET_PROPERTY {
 ...
 $self->{$name} = $newval;
 if ( (defined($newval) and defined($oldval) and $newval ne $oldval) or (defined($newval) xor defined($oldval))) {
   given ( $name ) {
     ...
     when ('page_number_start')        { $self->signal_emit('changed-page-number-start', $newval) }
     when ('page_number_increment')        { $self->signal_emit('changed-page-number-increment', $newval) }
...
#    $SETTING{'pages to scan'} = 'all';
   ...
   if ( $self->get('page-number-start') == 1 and $self->get('page-number-increment') < 0 ) {
...
#     if ( $SETTING{'cycle sane handle'} );
  ...
  new_page_callback => sub {
   ...
   $self->signal_emit('changed-progress', 0, make_progress_string( $i, $npages ));
  },
  error_callback => sub {
   my ($msg) = @_;
...
sub make_progress_string {
 my ( $i, $npages ) = @_;
 return sprintf $d->get("Scanning page %d of %d"), $i, $npages if ( $npages > 0 );
